...
public class KodoUnderFileSystem extends ObjectUnderFileSystem {
    ...
    protected KodoUnderFileSystem(AlluxioURI uri, KodoClient kodoclient, String bucketname, UnderFileSystemConfiguration conf) {
        super(uri, conf);
        mKodoClinet = kodoclient;
        mBucketName = bucketname;
    }

    public static KodoUnderFileSystem creatInstance(AlluxioURI uri, UnderFileSystemConfiguration conf) {
    ...
    protected ObjectListingChunk getObjectListingChunk(String key, boolean recursive) throws IOException {
        LOG.info("Qiniu: this is qiniu list first");
    ...
    private FileListing getObjectListingChunk(String prefix, int limit, String delimiter) {
        //LOG.info("Qiniu: this is qiniu list second");
    ...
    private final class KodoObjectListingChunk implements ObjectListingChunk {
        ...
        public ObjectStatus[] getObjectStatuses() {
            ...
            LOG.info("Qiniu:get objectsStatus ");
            FileInfo[] fileInfos = mResult.items;
            ObjectStatus[] ret = new ObjectStatus[fileInfos.length];
            int i = 0;
            for (FileInfo fileInfo : fileInfos) {
                if (fileInfo.key!=null){
                    ret[i++] = new ObjectStatus(fileInfo.key, fileInfo.hash, fileInfo.fsize, fileInfo.putTime/10000);
                }
            }
            int j = 0;
            for (;j<ret.length;j++){
                LOG.info("retinfo ---------");
                LOG.info(ret[j].getName());
                LOG.info(ret[j].getContentHash());
                LOG.info(String.valueOf(ret[j].getContentLength()));
                LOG.info(String.valueOf(ret[j].getLastModifiedTimeMs()));
            }
            return ret;
        }
        ...
        public ObjectListingChunk getNextChunk() throws IOException {
            ...
            if (!mResult.isEOF()) {
                FileListing nextResult = mKodoClinet.listFiles(mBucketName, mprefix, mResult.marker, mlimit, mdelimiter);
